...
public class SigmoidBPNN {
    ...
    private double errLimit = 0.000001;


    public SigmoidBPNN(int iptNum, int hdnNum, int optNum) {
    ...
    public SigmoidBPNN(String fileName) throws IOException {
        readConf(fileName);
    }
    ...
    private void randomWeights() {
        ...
        for(int i = 0; i < inputNum; i++) {
            for(int j = 0; j< hiddenNum; j++) {
                weight_ih[i][j] = (Math.random()-0.5)*2/inputNum;
                //System.out.println(weight_ih[i][j]);
            }
        }
        for(int i = 0; i < hiddenNum; i++) {
            for(int j = 0; j< outputNum; j++) {
                weight_ho[i][j] = (Math.random()-0.5)*2/hiddenNum;
        ...
        for(int i = 0; i < inputNum; i++) {
            ...
            for(int j = 0; j< hiddenNum; j++) {
                ...
                for(int k = 0; k< outputNum;k++) {
                    ...
                    sum += weight_ho[j][k]*output[k]*(1-output[k])*(desire[k]-output[k]);
                }
                weight_ih[i][j] += learnRate*hidden[j]*(1-hidden[j])*input[i]*sum;
            }
        }
    }
    ...
    private void adjustW_ho() {
        ...
        for(int i = 0; i < outputNum; i++) {
            for(int j = 0; j< hiddenNum; j++) {
                weight_ho[j][i] += learnRate*(desire[i]-output[i])*output[i]*(1-output[i])*hidden[j];
            }
        }
    }
    ...
    private void adjustBias1() {
        for(int j = 0; j< hiddenNum; j++) {
            ...
            bias1[j] += learnRate*hidden[j]*(1-hidden[j])*sum;
        }
    ...
    private void adjustBias2() {
        for(int i = 0; i < outputNum; i++) {
            bias2[i] += learnRate*(desire[i]-output[i])*output[i]*(1-output[i]);
        }
    }
